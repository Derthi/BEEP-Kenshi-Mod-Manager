...
function detectConflicts(activeMods, gamePath) {
  ...
  const perModEdits = {}; // modFilename -> { exterior: {}, interior: {} } (every edit, for the map "All Edits" mode)
  ...
  for (const { lf, touches, prefix } of parsedLevels) {
    ...
    for (const t of touches) {
      ...
      // Harvest per-mod edits (every edit, not just conflicts) for the map "All Edits" mode.
      // Free: this runs in the same pass that already resolves zones, before the conflict filter below.
      const layer = isInteriors ? 'interior' : 'exterior';
      if (!perModEdits[lf.modFilename]) perModEdits[lf.modFilename] = { exterior: {}, interior: {} };
      if (zones) {
        const layerMap = perModEdits[lf.modFilename][layer];

...
// Resolve a child directory by name, case-insensitively. Kenshi's local mods folder is "mods",
// but the code historically looked for "Mods" — identical on Windows, but a mismatch on
// case-sensitive filesystems (Linux). Match any case so it works everywhere.
function resolveDir(parent, name) {
  ...
  try {
    ...
    // parent missing or unreadable — fall through to the requested path
  }
  return direct;
}
...
function discoverMods(gamePath, steamModsPath) {
  ...
  const localPath = resolveDir(gamePath, 'Mods');
